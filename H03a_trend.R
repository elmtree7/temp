...
# 9/24/2025
##################################################################################################### 
# library(quantmod) This does not work at the moment
library(tseries)
library(fpp2)
library(ggplot2)
library(xts)
...
# You need to get an Fred API key first -- https://fred.stlouisfed.org/docs/api/api_key.html
# fredr_set_key("8ba88b5e6908c85ddeaff9993a4b4aaa") # This is my key. Don't use it. Use your own.

# Real GDP: https://fred.stlouisfed.org/series/GDPC1
# Reference: https://rpubs.com/DanielSLee/ImportingManagingFinancialData

x = fredr_series_observations("GDPC1")
gdp_ts = xts(x$value, order.by = x$date)
plot(gdp_ts)
head(gdp_ts); tail(gdp_ts)
plot(log(gdp_ts))

# Real GDP Growth Rate; https://fred.stlouisfed.org/series/A191RL1Q225SBEA/
x = fredr_series_observations("A191RL1Q225SBEA")
gdpg_ts = xts(x$value, order.by = x$date)
plot(gdpg_ts)

gdp = data.frame(gdp_ts)
trend = data.frame(c(1:length(gdp_ts)))
trend2 = trend^2
lrgdp = data.frame(log(gdp_ts))
...
ts.plot(gdp_ts, fit01.ts, col = c("black","red"))
...
ts.plot(gdp_ts, fit02.ts, lty=c(1,2), lwd=c(1,2))
...
ts.plot(gdp_ts, exp(fit03.ts), lty=c(1,5), lwd=c(1,3),col=c("black","blue"))
